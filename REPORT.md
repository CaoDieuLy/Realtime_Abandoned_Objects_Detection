...
Hệ thống có **ba lane** chạy ở **ba nhịp KHÁC nhau**, không phải ba nhánh song song cùng-nhịp: **① khởi động một-lần** dựng nền-sạch rồi đóng-băng; **② vòng per-frame** (ViBe + diff-nền → FSM → cảnh báo); **③ ngữ nghĩa bất-đồng-bộ** (YOLO chạy ở luồng nền, KHÔNG mỗi frame). Hình 1 vẽ đúng ba nhịp này.

**Hình 1 — Kiến trúc theo ba nhịp (đã sửa: nền-sạch là *đầu-vào đóng-băng* dựng lúc khởi động, KHÔNG ngang-hàng ViBe/YOLO; YOLO chạy *async* không mỗi frame).**
```mermaid
graph TD
    subgraph INIT["① KHỞI ĐỘNG — chạy MỘT LẦN (warm-up ~20s đầu)"]
        WF[Khung warm-up] --> CB["clean_bg = median ĐÓNG BĂNG<br/>(nền-sạch dài hạn, nền tham chiếu)"]
    end
    subgraph ASY["③ NGỮ NGHĨA — luồng NỀN, KHÔNG mỗi frame (~mỗi N frame)"]
        YOLO["YOLO-seg (async worker)"] --> SMAP["semantic map (cache)<br/>animate: loại người/xe · object: giữ balo/túi"]
    end
    subgraph LOOP["② MỖI KHUNG HÌNH — per-frame"]
        FR[Khung hình t] --> VB["ViBe/pybgs → mask moving"]
        FR --> ND["newdiff = |frame − clean_bg|"]
        VB --> FSM["StaticForegroundState FSM<br/>static_fg = newdiff ∧ ¬moving → tuổi-tĩnh"]
        ND --> FSM
        FSM --> MATCH["abandoned (age ≥ T) → StaticMatcher"]
        MATCH --> GATE["Cổng cảnh báo<br/>evidence · dedup · owner · light-struct<br/>(+ CLIP nếu Luồng 2)"]
        GATE --> EVENT["Sự kiện + JSON → GUI"]
    end
    CB -. "nền tham chiếu (đóng băng)" .-> ND
    FR -. "frame mới nhất" .-> YOLO
    SMAP -. "map GẦN NHẤT (cũ vài frame)" .-> FSM
```

Đọc sơ đồ: **nền-sạch (`clean_bg`) KHÔNG là một nhánh per-frame ngang-hàng ViBe/YOLO** — nó được dựng **một lần** ở giai-đoạn khởi-động (median warm-up) rồi **đóng-băng**, sau đó chỉ là **đầu-vào tham chiếu** cho phép trừ `|frame − clean_bg|` mỗi khung (mũi tên đứt). **YOLO-seg cũng KHÔNG chạy mọi khung** — nó ở **luồng nền bất-đồng-bộ** (~mỗi N frame; native nhả GIL nên song song với vòng chính), vòng chính chỉ đọc **bản-đồ ngữ-nghĩa gần-nhất** đã có sẵn (cache). Ba khối chạy mỗi-frame thực-sự là: **ViBe** (mask-moving), **diff-nền** (newdiff), **FSM→matcher→cổng**. Nguồn ngữ nghĩa **cắm-thay-được** (YOLO-seg / SegFormer / PSPNet). **Hai luồng vận hành** khác nhau chỉ ở **cổng cảnh-báo cuối**: Luồng-1 (không-CLIP, mặc định) và Luồng-2 (thêm CLIP-verify, opt-in) — bảng so sánh ở §9.
...
### 9.0. Hai luồng vận hành (no-CLIP vs CLIP)

Repo chạy được **hai luồng**, **giống hệt nhau** ở toàn bộ §1–§8 và **cùng bật async** (§9.1), **chỉ khác ở cổng cảnh-báo cuối** — Luồng-2 thêm một bước xác-minh CLIP:

**Hình 14 — Cổng cảnh-báo cuối tách hai luồng.**
```mermaid
flowchart LR
    C["ứng-viên tĩnh ≥ T<br/>(qua evidence·dedup·owner·light-struct)"] --> S{"--clip-verify ?"}
    S -->|"0 — Luồng 1 (mặc định)"| A1["★ CẢNH BÁO"]
    S -->|"1 — Luồng 2 (opt-in)"| CL["CLIP verify crop<br/>object → giữ<br/>non-object → loại"]
    CL --> A2["★ CẢNH BÁO"]
```

**Bảng 7 — So sánh hai luồng trên ABODA (13 video, 1 cấu hình mặc định).**
...
**Hình 15 — FP mỗi video: baseline vs async+CLIP.**

![fp per video](report_figures/fig_asyncclip_fp.png)

**Hình 16 — Tổng kết Recall & FP (11 video GT).**

![summary](report_figures/fig_asyncclip_summary.png)

**Hình 17 — Tốc độ mỗi video (async + CLIP on, CPU).**

![fps](report_figures/fig_asyncclip_fps.png)

**Bảng 8 — Full-sweep per-video: baseline (§6.2) vs Luồng-2 (async+CLIP).**
